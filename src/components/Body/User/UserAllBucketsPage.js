...
const BucketsPage = () => {
  ...
  const createBucket = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(process.env.BACKENDAPI + `/user/${cUserId}/bucket/create`,formData, {
        withCredentials: true,
      });
      ...
      <div className="container mx-auto py-8 ">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 items-center md:px-[5%]">
          <div className=" rounded-lg p-6 shadow-md bg-gray-200">
            ...
            ></input>
            <button onClick={createBucket} className="bg-green-400 p-2 rounded-md mt-2 w-full">Create</button>
          </div>
          {buckets?.map((bucket, index) => (
            <Link to={`/bucket/${bucket?._id}`} key={index}>
              <div className="bg-white rounded-lg p-6 shadow-md">
                ...
                <p className="text-gray-700">
                  Created At:{" "}
                  {bucket?.createdAt.slice(0, 10) + "/" + timeConverter(bucket?.createdAt.slice(11))}
                </p>
                <p className="text-gray-700">
                  Updated At:{" "}
                  {bucket?.updatedAt.slice(0, 10) + "/" + timeConverter(bucket?.updatedAt.slice(11))}
                </p>
              </div>
            </Link>
          ))}
